...
第 16 课已经能注册、鉴权、操作项目并运行 E2E，但部署平台还不知道应用是否健康，HTTP 响应缺少安全头，接口也没有基础限流。不要把这些问题误认为业务 Service 的职责。
...
- Helmet 设置常用安全响应头，但它不是业务授权系统。
- CORS 控制浏览器能否从指定来源读取响应，不阻止 curl 或服务端请求。
- Throttler 降低滥用和暴力尝试风险，不代替账号锁定、WAF 或容量规划。
