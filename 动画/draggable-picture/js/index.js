...
function dragContainerMouseMove(e) {
	...
	if (imageBox.canDrag.h)
		var newx = dx > 0 ? Math.min(0, dragPicture.point.left + dx) : Math.max(-dragPicture.pictureSize.w + imageBox
			.imageBoxSize.w, dragPicture.point.left + dx);
	if (imageBox.canDrag.v)
		var newy = dy > 0 ? Math.min(0, dragPicture.point.top + dy) : Math.max(-dragPicture.pictureSize.h + imageBox
			.imageBoxSize.h, dragPicture.point.top + dy);
